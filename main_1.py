...
def federated_learning():
    ...
    # 数据分布配置
    dirichlet_alpha = 1  # 狄利克雷分布参数，控制数据异质性
    ...
    dataset_name = 'cifar10'  # 可选：'mnist', 'cifar10', 'cifar100'
    ...
    if os.path.exists('./cache1/global_model_state.pkl'):
        try:
            server1_model.load_state_dict(torch.load('./cache1/global_model_state.pkl', map_location=device))
        except:
            pass
    ...
    if enable_external_attacker:
        ...
        if os.path.exists('./cache1/global_model_state.pkl'):
            try:
                attack_model.load_state_dict(torch.load('./cache1/global_model_state.pkl', map_location=device))
            except:
                pass
    ...
    for e in range(n_epoch):
        ...
        for i, c in enumerate(clients):
            if i not in forgotten_clients:
                ...
                if external_attacker is not None:
                    try:
                        client_grads = torch.load(f'./cache1/grads_{i}.pkl', map_location='cpu')
                        if logger:
        ...
        for s1 in server1s:
            ...
            if external_attacker is not None:
                try:
                    server1_grads = torch.load(f'./cache1/grads_agg1_{s1.rank}.pkl', map_location='cpu')
