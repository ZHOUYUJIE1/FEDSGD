...
class GradientInversionAttacker:
    ...
    def _compute_gradient_loss_soft(self, dummy_data, dummy_target_probs, gradients):
        """计算梯度匹配损失（soft label版本，允许对标签进行可微优化）

        dummy_target_probs: shape [B, C]，每行和为1（通常由softmax(dummy_logits)得到）
        """
        # 清除模型参数的梯度
        for param in self.model.parameters():
            if param.grad is not None:
        ...
        # 前向传播
        output = self.model(dummy_data)  # [B, C]

        # soft cross entropy: -sum(p * log q)
        log_q = F.log_softmax(output, dim=1)
        loss = -(dummy_target_probs * log_q).sum(dim=1).mean()
        ...
        for i, (name, param) in enumerate(self.model.named_parameters()):
            if name in gradients and grads[i] is not None:
                ...
                if isinstance(true_grad, torch.Tensor):
                    ...
                    if grads[i].shape == true_grad.shape:
                        mse_loss = F.mse_loss(grads[i], true_grad)
                        cos_sim = F.cosine_similarity(grads[i].flatten(), true_grad.flatten(), dim=0)
                        grad_loss += mse_loss + (1 - cos_sim)
                        count += 1
    ...
    def perform_gradient_inversion(self, gradients: Dict, epoch: int, client_rank: int = None,
        ...
        try:
            ...
            # 在DLG开始时保存一次GT（如果提供了original_data）
            if original_data is not None and original_labels is not None:
                self._save_ground_truth_only(original_data, original_labels, epoch, client_rank, "deep_leakage")
            ...
            for iteration in range(num_iterations):
                ...
                # 总损失
                total_loss = grad_loss + 0.0001 * tv_loss
    ...
    def perform_deep_leakage(self, gradients: Dict, epoch: int, client_rank: int = None,
                            batch_size: int = 1, num_iterations: int = 3000,
                            lr: float = 0.1, original_data: torch.Tensor = None,
                            original_labels: torch.Tensor = None) -> Dict:
        ...
        # === 强制冻结 BN / Dropout：让攻击端前向行为更稳定 ===
        self.model.eval()
        for m in self.model.modules():
            if isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.Dropout, nn.Dropout2d, nn.Dropout3d)):
                m.eval()
        ...
        try:
            ...
            dummy_data = nn.Parameter(dummy_data)
            # 代替 dummy_data 直接优化
            # dummy_z = torch.randn_like(original_data, requires_grad=True)
            # dummy_data = torch.tanh(dummy_z)  # 自动限制范围到 [-1,1]，梯度更平滑
            ...
            for iteration in range(num_iterations):
                ...
                # 从logits获取soft标签分布（可微），同时保留一个离散标签用于日志/可视化
                dummy_probs = F.softmax(dummy_logits, dim=1)              # [B, C]
                dummy_labels = dummy_probs.argmax(dim=1)                 # [B] (仅用于展示/评估)

                # 计算梯度匹配损失（soft-label版本，确保dummy_logits能被优化）
                grad_loss = self._compute_gradient_loss_soft(dummy_data, dummy_probs, gradients)
                ...
                # 总损失（根据DLG论文）
                total_loss = grad_loss + 0.0001 * tv_loss + 0.001 * label_entropy
    ...
    def _save_ground_truth_only(self, original_data: torch.Tensor, original_labels: torch.Tensor,
                            epoch: int, client_rank: int, attack_type: str):
        """只保存攻击目标（GT）图像，不依赖 reconstruction，也不受 save_reconstruction_only / iteration_step 影响。"""
        import os
        import matplotlib.pyplot as plt
        import torch
        ...
        # 只保存一次：epoch + identifier + attack_type
        key = (epoch, identifier, attack_type)
        if not hasattr(self, "saved_original_images"):
            self.saved_original_images = set()
        if key in self.saved_original_images:
            return

        # 反标准化/裁剪到可视范围（用你现有的后处理）
        original_vis = self._postprocess_image_for_vis(original_data)

        # numpy 形状处理
        num_samples = min(original_vis.size(0), 8)
        if original_vis.size(1) == 1:
            original_vis_np = original_vis.squeeze(1).cpu().numpy()   # [B,H,W]
            cmap = "gray"
        else:
            original_vis_np = original_vis.permute(0, 2, 3, 1).cpu().numpy()  # [B,H,W,C]
            cmap = None
        ...
        for i in range(num_samples):
            axes[i].imshow(original_vis_np[i], cmap=cmap, interpolation="bicubic")
            axes[i].set_title(f"GT\nlabel:{int(original_labels[i].item())}")
            axes[i].axis("off")

        plt.tight_layout()
        save_path = f"./cache/attack_results/{attack_type}_epoch{epoch}_{identifier}_ground_truth.png"
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
        plt.close(fig)

        self.saved_original_images.add(key)
        if self.logger:
            self.logger.info(f"  [GT] 已保存攻击目标图像: {save_path}")
    ...
    def _visualize_reconstruction(self, original_data: torch.Tensor, reconstructed_data: torch.Tensor,
        ...
        try:
            ...
            if should_save_ground_truth and num_samples > 0:
                # 保存地面真实图像
                fig_ground_truth, axes_gt = plt.subplots(1, num_samples, figsize=(2*num_samples, 2))
                if num_samples == 1:
                    axes_gt = [axes_gt]
                for i in range(num_samples):
            ...
            if num_samples > 0:
                ...
                for i in range(num_samples):
                    axes_recon[i].imshow(reconstructed_vis_np[i], cmap=cmap)
...
class InternalAttacker(GradientInversionAttacker):
    ...
    def _find_last_linear_grad_keys(self, gradients: dict):
        """
        从 gradients(dict[name -> tensor]) 里找“最后一层分类头”的 weight/bias key。
        兼容：
        - LeNet5:  fc.2.weight / fc.2.bias
        - ResNet18: fc.weight / fc.bias
        更通用：优先选 shape[0] == n_class 的那组 linear 梯度作为最后一层。
        """
        # 1) 先收集所有疑似 linear weight
        weight_candidates = []
        for k, v in gradients.items():
            if v is None or not hasattr(v, "shape"):
                continue
            # 线性层 weight 通常是 2D
            if v.dim() == 2 and ("fc" in k or "classifier" in k or "linear" in k) and k.endswith("weight"):
                weight_candidates.append((k, v))

        if not weight_candidates:
            return None, None

        # 2) 优先选 out_features == n_class 的那一个
        for k, v in weight_candidates:
            try:
                if int(v.shape[0]) == int(self.n_class):
                    bias_k = k.replace("weight", "bias")
                    return k, bias_k if bias_k in gradients else None
            except Exception:
                pass

        # 3) 兜底：选“最靠后”的 fc.*.weight（按名字排序）
        weight_candidates.sort(key=lambda x: x[0])
        k, _ = weight_candidates[-1]
        bias_k = k.replace("weight", "bias")
        return k, bias_k if bias_k in gradients else None


    def _log_last_linear_grad_stats(self, gradients: dict, epoch=None, stage="unknown"):
        """
        打印最后一层 fc/linear 的 weight/bias 梯度：shape + min/max/mean/std + L2 norm
        """
        try:
            w_key, b_key = self._find_last_linear_grad_keys(gradients)
            if w_key is None:
                msg = f"[Attack Debug] epoch={epoch} stage={stage} cannot find last linear grad keys."
            ...
            if b_key is not None and b_key in gradients and gradients[b_key] is not None:
                b = gradients[b_key]
                b_s = _stat(b)
            else:
                b_s = None

            msg = (f"[Attack Debug] epoch={epoch} stage={stage} "
                f"LAST_LINEAR weight={w_key} shape={w_s['shape']} "
                f"min={w_s['min']:.6g} max={w_s['max']:.6g} mean={w_s['mean']:.6g} std={w_s['std']:.6g} l2={w_s['l2']:.6g}")
            if self.logger: self.logger.info(msg)
            else: print(msg)

            if b_s is not None:
                msg2 = (f"[Attack Debug] epoch={epoch} stage={stage} "
                        f"LAST_LINEAR bias={b_key} shape={b_s['shape']} "
                        f"min={b_s['min']:.6g} max={b_s['max']:.6g} mean={b_s['mean']:.6g} std={b_s['std']:.6g} l2={b_s['l2']:.6g}")
                if self.logger: self.logger.info(msg2)
                else: print(msg2)

        except Exception as e:
            msg = f"[Attack Debug] epoch={epoch} stage={stage} failed to log last linear grad stats: {e}"
            if self.logger: self.logger.warning(msg)
            else: print(msg)
    ...
    def attack_on_gradients(self, gradients: Dict, epoch: int, client_rank: int = None,
                           ...
                           original_labels: torch.Tensor = None) -> Dict:
        """对接收到的梯度进行攻击"""
        self._log_last_linear_grad_stats(gradients, epoch=epoch, stage="input_gradients")
